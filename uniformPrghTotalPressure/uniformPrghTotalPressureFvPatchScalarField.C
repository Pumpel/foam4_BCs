...
#include "uniformPrghTotalPressureFvPatchScalarField.H"
...
uniformPrghTotalPressureFvPatchScalarField
(
    const uniformPrghTotalPressureFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::uniformPrghTotalPressureFvPatchScalarField::updateCoeffs
(
...
)
{
    ...
    // Added to support hydrostatic pressure
    ...
    if (internalField().dimensions() == dimPressure)
    {
        if (psiName_ == "none")
        {
            ...
            operator==(p0 - 0.5*rho*(1.0 - pos(phip))*magSqr(Up)
            		- rho*((g.value() & patch().Cf()) - ghRef.value()));
        }
        else
        {

        	FatalErrorInFunction
				<< "High-speed compressible flow is not supported by uniformPrgTotalPressure."
				<< exit (FatalError);
...
void Foam::uniformPrghTotalPressureFvPatchScalarField::write(Ostream& os) const
{
