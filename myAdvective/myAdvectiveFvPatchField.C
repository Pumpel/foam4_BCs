...
#include "myAdvectiveFvPatchField.H"
...
template<class Type>
Foam::myAdvectiveFvPatchField<Type>::myAdvectiveFvPatchField
(
    const fvPatch& p,
...
Foam::myAdvectiveFvPatchField<Type>::myAdvectiveFvPatchField
(
    const myAdvectiveFvPatchField& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
Foam::myAdvectiveFvPatchField<Type>::advectionSpeed() const
{
...
void Foam::myAdvectiveFvPatchField<Type>::updateCoeffs()
{
    ...
    if (lInf_ > 0)
    {
        ...
        )
        {
            this->refValue() =
            (
                field.oldTime().boundaryField()[patchi] + k*fieldInf_->value(t)
            )/(1.0 + k);
